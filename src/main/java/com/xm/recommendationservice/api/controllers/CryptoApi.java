...
public interface CryptoApi {
        ...
        @Operation(summary = "Get stats (oldest, newest, min, max) for a crypto", description = "Optionally provide a date range to filter the results. Both start and end dates must be provided if filtering by date.", responses = {
                        ...
                        @ApiResponse(responseCode = "404", description = "Crypto not found", content = @Content(mediaType = "application/json", schema = @Schema(implementation = ApiErrorResponse.class), examples = @ExampleObject(name = "Crypto Not Found", value = """
                                        ...
                                        """))),
                        @ApiResponse(responseCode = "400", description = "Invalid input or missing required parameters", content = @Content(mediaType = "application/json", schema = @Schema(implementation = ApiErrorResponse.class), examples = @ExampleObject(name = "Missing Dates", value = """
                                        {
        ...
        CryptoNormalizedRange getTopCryptoForDay(
        ...
        List<CryptoNormalizedRange> getAllSortedByNormalizedRange(
                        @Parameter(description = "Start date (optional)") @RequestParam(value = "startDate", required = false) @DateTimeFormat(iso = DateTimeFormat.ISO.DATE) LocalDate startDate,
                        @Parameter(description = "End date (optional)") @RequestParam(value = "endDate", required = false) @DateTimeFormat(iso = DateTimeFormat.ISO.DATE) LocalDate endDate);
}
